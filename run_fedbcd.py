...
from algorithm.fedbcd import fedbcd
...
if __name__ == '__main__':
    ...
    # task
    task_config = toml.load('./config/fedbcd.toml')
    ...
    log_path = './train_log/log_fedbcd.txt'
    result_path = './output/result_fedbcd.csv'
    epoch_path = './output/epoch_fedbcd.csv'
    ...
    for dataset in dataset_list:
        ...
        for seed, k in tqdm(product(seeds, k_list), total=len(k_list) * len(seeds), desc=dataset):
            if k == 1.0:
                k = None
            result_info, epoch_info_list = fedbcd(dataset, lr, epochs, embed_dim, client_num, train_loader, test_loader, seed, device, Q, c_factor=b_dict[k], debug_flag=debug_flag)
