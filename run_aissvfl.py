...
from algorithm.aissvfl import aissvfl
...
if __name__ == '__main__':
    ...
    # task
    task_config = toml.load('./config/aissvfl.toml')
    k_list = task_config['k_list']
    tau_list = task_config['tau_list']

    
    os.chdir(work_dir)

    
    log_path = './train_log/log_aissvfl.txt'
    result_path = './output/result_aissvfl.csv'
    epoch_path = './output/epoch_aissvfl.csv'
    ...
    for dataset in dataset_list:
        ...
        for seed, k, tau in tqdm(product(seeds, k_list, tau_list), total=len(k_list) * len(seeds) * len(tau_list), desc=dataset):
            if k == 1.0:
                k = None

            result_info, epoch_info_list = aissvfl(dataset, lr, epochs, embed_dim, client_num, train_loader, test_loader, seed, device, tau, c_factor=b_dict[k], debug_flag=debug_flag)
