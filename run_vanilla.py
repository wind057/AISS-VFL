...
if __name__ == '__main__':
    ...
    # task
    task_config = toml.load('./config/vanilla.toml')
    ...
    log_path = './train_log/log_vanilla.txt'
    result_path = './output/result_vanilla.csv'
    epoch_path = './output/epoch_vanilla.csv'
    ...
    for dataset in dataset_list:
        ...
        for seed, k in tqdm(product(seeds, k_list), total=len(k_list) * len(seeds), desc=dataset):
            ...
            result_info, epoch_info_list = vfl(dataset, lr, epochs, embed_dim, client_num, train_loader, test_loader, seed, device,
                                            c_type='topk', c_factor=k, debug_flag=debug_flag)
            result_list.append(result_info)
            epoch_list.extend(epoch_info_list)
            if k is None:
                continue
            result_info, epoch_info_list = vfl(dataset, lr, epochs, embed_dim, client_num, train_loader, test_loader, seed, device,
                                            c_type='scalar', c_factor=b_dict[k], debug_flag=debug_flag)
            result_list.append(result_info)
            epoch_list.extend(epoch_info_list)
